...
public enum GvrControllerHand
{
    ...
    // Alias for dominant hand as specified by `GvrSettings.Handedness`.
    Dominant,

    // Alias for non-dominant hand.
    NonDominant,
}
...
public class GvrControllerInput : MonoBehaviour
{
    ...
    public static string ErrorDetails
    {
        get
        {
            ...
            {
                return "No GvrControllerInput initialized instance found in scene. It may be missing, or it might not have initialized yet.";
            }
        }
    }
    ...
    void Awake()
    {
        if (instances.Length > 0)
        {
            Debug.LogError("More than one active GvrControllerInput instance was found in your scene. Ensure that there is only one GvrControllerInput.");
            this.enabled = false;
            return;
        }
