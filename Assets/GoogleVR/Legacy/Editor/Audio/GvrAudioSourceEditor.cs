//-----------------------------------------------------------------------
// <copyright file="GvrAudioSourceEditor.cs" company="Google Inc.">
...
public class GvrAudioSourceEditor : Editor
{
    ...
    private GUIContent clipLabel = new GUIContent(
        "AudioClip", "The AudioClip asset played by the GvrAudioSource.");

    private GUIContent loopLabel = new GUIContent(
        "Loop", "Sets the source to loop.");
    ...
    private GUIContent spatialBlendLabel = new GUIContent(
        "Spatial Blend",
        "Sets how much this source is treated as a 3D source. Setting this value to 0 will ignore distance attenuation and doppler effects. However, it does not affect panning the sound around the listener.");
    ...
    private GUIContent dopplerLevelLabel = new GUIContent(
        "Doppler Level",
        "Specifies how much the pitch is changed based on the relative velocity between the source and the listener.");

    private GUIContent spreadLabel = new GUIContent(
        "Spread", "Source spread in degrees.");
    ...
    private GUIContent minDistanceLabel = new GUIContent(
        "Min Distance",
        "Within the min distance, the volume will stay at the loudest possible. Outside this min distance it will begin to attenuate.");

    private GUIContent playOnAwakeLabel = new GUIContent(
        "Play On Awake", "Play the sound when the scene loads.");

    private GUIContent bypassRoomEffectsLabel = new GUIContent(
        "Bypass Room Effects",
        "Sets whether the room effects for the source should be bypassed.");

    private GUIContent directivityLabel = new GUIContent(
        "Directivity",
        "Controls the pattern of sound emission of the source. This can change the perceived loudness of the source depending on which way it is facing relative to the listener. Patterns are aligned to the 'forward' direction of the parent object.");

    private GUIContent directivityAlphaLabel = new GUIContent(
        "Alpha",
        "Controls the balance between dipole pattern and omnidirectional pattern for source emission. By varying this value, differing directivity patterns can be formed.");

    private GUIContent directivitySharpnessLabel = new GUIContent(
        "Sharpness",
        "Sets the sharpness of the directivity pattern. Higher values will result in increased directivity.");

    private GUIContent listenerDirectivityLabel = new GUIContent(
        "Listener Directivity",
        "Controls the pattern of sound sensitivity of the listener for the source. This can change the perceived loudness of the source depending on which way the listener is facing relative to the source. Patterns are aligned to the 'forward' direction of the listener.");

    private GUIContent listenerDirectivityAlphaLabel = new GUIContent(
        "Alpha",
        "Controls the balance between dipole pattern and omnidirectional pattern for listener sensitivity. By varying this value, differing directivity patterns can be formed.");

    private GUIContent listenerDirectivitySharpnessLabel = new GUIContent(
        "Sharpness",
        "Sets the sharpness of the listener directivity pattern. Higher values will result in increased directivity.");

    private GUIContent gainLabel = new GUIContent(
        "Gain(dB)",
        "Applies a gain to the source for adjustment of relative loudness.");

    private GUIContent hrtfEnabledLabel = new GUIContent(
        "Enable HRTF",
        "Sets HRTF binaural rendering for the source. Note that this setting has no effect when stereo quality mode is selected globally.");

    private GUIContent occlusionLabel = new GUIContent(
        "Enable Occlusion",
        "Sets whether the sound of the source should be occluded when there are other objects between the source and the listener.");
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (rolloffMode.enumValueIndex == (int)AudioRolloffMode.Custom)
        {
            EditorGUILayout.HelpBox(
                "Custom rolloff mode is not supported, no distance attenuation will be applied.",
                MessageType.Warning);
        }
