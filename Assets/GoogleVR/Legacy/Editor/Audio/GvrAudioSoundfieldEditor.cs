//-----------------------------------------------------------------------
// <copyright file="GvrAudioSoundfieldEditor.cs" company="Google Inc.">
...
public class GvrAudioSoundfieldEditor : Editor
{
    ...
    private GUIContent clip0102Label = new GUIContent(
            "Channels 1 & 2 (WY)",
            "The AudioClip asset for the 1 & 2 channels (W & Y components) of the first-order ambisonic soundfield. Channels must be in Ambix (ACN/SN3D) format.");

    private GUIContent clip0304Label = new GUIContent(
            "Channels 3 & 4 (ZX)",
            "The AudioClip asset for the 3 & 4 channels (Z & X components) of the first-order ambisonic soundfield. Channels must be in Ambix (ACN/SN3D) format.");
    ...
    private GUIContent priorityLabel = new GUIContent(
            "Priority",
            "Sets the priority of the soundfield. Note that a sound with a larger priority value will more likely be stolen by sounds with smaller priority values.");

    private GUIContent spatialBlendLabel = new GUIContent(
            "Spatial Blend",
            "Sets how much this soundfield is treated as a 3D source. Setting this value to 0 will ignore distance attenuation and doppler effects. However, it does not affect panning the sound around the listener.");
    ...
    private GUIContent dopplerLevelLabel = new GUIContent(
            "Doppler Level",
            "Specifies how much the pitch is changed based on the relative velocity between the soundfield and the listener.");
