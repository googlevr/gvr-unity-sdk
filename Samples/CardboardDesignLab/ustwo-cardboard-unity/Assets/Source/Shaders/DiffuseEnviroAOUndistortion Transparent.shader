...
Shader "UsTwo-Cardboard/Diffuse Environment AO Undistortion Transparent" {
	...
	SubShader {
		Pass {
			...
			{  
				...
				half diffuse = tex2D(_Diffuse, fragment.uv).a;
				...
				//shadowing = ;
				half3 color = _Color.rgb*(ambient + lighting)*(fragment.NDotL_Rim.w);
				color = fragment.fog.rgb + color;

				return half4(color, _Color.a*diffuse);
