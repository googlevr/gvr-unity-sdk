...
Shader "UsTwo-Cardboard/Diffuse Environment AO Undistortion Solid Color Menu 1" {
	...
	SubShader {
		Pass {
			...
			{  
				...
				half3 lighting = saturate(ao + 0.5)*fragment.NDotL_Rim.xyz*(shadowing  + (1-shadowing)*_ShadowColor );//diffuse*_PrimaryLightColor.rgb
				...
				return (1-_Color.a)*half4(1,1,1,1) + (_Color.a)*half4(color, 1);//half4(color, 1);
