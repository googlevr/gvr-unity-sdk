...
Shader "UsTwo-Cardboard/Transparent Color Undistortion Rounded Corners Background" {
	...
	SubShader {
		Pass {
			...
			{  
				...
				half2 dist = abs(fragment.uv); 
				half2 pixels = abs(fragment.vertexPosition.xz); 

				half2 norm =  pixels/(dist);

				half2 edgeDist = abs(pixels-norm);
				...
				return half4(_Color.rgb,_Color.a*alpha);// half4(_Color.rgb, dist.x);
			}
			ENDCG
		}
	}
